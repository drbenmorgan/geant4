...
/** @brief Physics vector specialised for general bins
*
* Supports bin edges with any spacing, as long as these are increasing (consecutive edges may be equal)
* @check Interpolation will always select the lower-numbered bin in case consecutive energies are set equal
*/
...
public:
  ...
  /** @brief Construct vector of given length initialised to 0 */
  ...
  /** @brief Construct and fill a vector
  *
  ...
  * @post Energy and value are filled
  ...
  ~G4PhysicsFreeVector() override = default;
    ...
    /**
     * @brief Put data for energy bin and value at a given index
  ...
  void PutValues(const std::size_t index, 
    ...
    /**
     * @brief Insert an extra pair of energy and value.
     *
  ...
  void InsertValues(const G4double energy, const G4double value);
    ...
    /**
     * @brief Enable logarithmic bin search.
     * @param n Number of bins (default 1)
     */
  void EnableLogBinSearch(const G4int n = 1);
  
    /**
     * @brief Obsolete. Fill the vector with a value at a given index and energy.
     * @deprecated
     * @param index Index in the vector
