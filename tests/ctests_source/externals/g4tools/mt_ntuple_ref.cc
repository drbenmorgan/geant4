...
// The "_ref" refers to the fact that each ntuple column handles a reference to a user variable
// which is used at fill time.
...
class thread_args : public tools::thread_args
{
  ...
  public:
    void _main()
    {
      ...
      // user variables used to create "ref columns" in the local thread ntuple and also used to
      // fill user data :
      double user_rgauss;
      float user_rbw;
      ...
      if (m_row_wise) {
        ...
        _ntuple = new tools::wroot::mt_ntuple_row_wise(
          m_out, main_file.byte_swap(), main_file.compression(),
          m_main_ntuple.dir().seek_directory(), *main_branch, main_branch->basket_size(),
          nbk_thread, m_verbose);
      }
      else {
        ...
        _ntuple = new tools::wroot::mt_ntuple_column_wise(
          m_out, main_file.byte_swap(), main_file.compression(),
          m_main_ntuple.dir().seek_directory(), main_branches, basket_sizes, nbk_thread, m_row_mode,
          m_nev, m_verbose);
      }
...

      {
        ...
        for (unsigned int count = 0; count < entries; count++) {
          ...
          if (!_ntuple->add_row(_mutex, main_file))
...
class threads : public tools::threads<thread_args>
{
  ...
  public:
                                     ...
                                     size_t /*a_num_threads*/)
    {
      ...
      prefix += ":";
      return new thread_args(a_main_mutex, a_index, /*a_num_threads,*/ prefix, m_nbk, m_main_ntuple,
                             m_row_wise, m_row_mode, m_nev, m_megas);
    }
...
int main(int argc, char** argv)
{
...

  {
    ...
    //////////////////////////////////////////////////////////
    {  // user variables used in the main ntuple to create "ref columns". (They are not used to fill
       // data):
      double user_rgauss;
      float user_rbw;
      std::string user_string;
