...
int main(int argc, char** argv)
{
  ...
  if (rank == 0) {
    ...
    std::vector<class_pointer> hs;
    {
      ...
      if (!_hmpi.wait_histos(rank_src, hs)) {
        std::cout << "rank " << rank << " : wait_histos from " << rank_src << " : failed."
                  << std::endl;
        return EXIT_FAILURE;
      }
  ...
  if (rank == 1) {
    ...
    tools::histo::h2d h2("h2d", 100, -5, 5, 100, -2, 2);
    {
      ...
      std::normal_distribution<double> rbw(0.0, 1.0);
      unsigned int entries = 10000;
      for (unsigned int count = 0; count < entries; count++)
        h2.fill(rg(gen), rbw(gen));
    }
