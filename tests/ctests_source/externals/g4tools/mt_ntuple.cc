...
class thread_args : public tools::thread_args
{
  ...
  public:
    thread_args(tools::mutex& a_main_mutex,
                ...
                const std::string& a_prefix, const tools::ntuple_booking& a_nbk,
                tools::wroot::ntuple& a_main_ntuple, bool a_row_wise, bool a_row_mode,
                unsigned int a_nev, unsigned int a_megas)
  ...
  private:  // thread_args not copiable.
    thread_args(const thread_args& a_from)
      : parent(a_from), m_index(a_from.m_index), m_main_ntuple(a_from.m_main_ntuple)
    {}
  ...
  public:
    void _main()
    {
      ...
      if (m_row_wise) {
        ...
        tools::wroot::mt_ntuple_row_wise* mt_ntuple = new tools::wroot::mt_ntuple_row_wise(
          m_out, main_file.byte_swap(), main_file.compression(),
          m_main_ntuple.dir().seek_directory(), *main_branch, main_branch->basket_size(), m_nbk,
          m_verbose);
        ...
        tools::wroot::mt_ntuple_column_wise* mt_ntuple = new tools::wroot::mt_ntuple_column_wise(
          m_out, main_file.byte_swap(), main_file.compression(),
          m_main_ntuple.dir().seek_directory(), main_branches, basket_sizes, m_nbk, m_row_mode,
          m_nev, m_verbose);
        _imt_ntuple = static_cast<tools::wroot::imt_ntuple*>(mt_ntuple);
        _base_pntuple = static_cast<tools::wroot::base_pntuple*>(mt_ntuple);
      }
      ...
      tools::wroot::base_pntuple::column<double>* col_rgauss =
        _base_pntuple->find_column<double>("rgauss");
      if (!col_rgauss) {
        m_out << "column rgauss not found." << std::endl;
        col_not_found = true;
      }
      tools::wroot::base_pntuple::column<float>* col_rbw = _base_pntuple->find_column<float>("rbw");
      if (!col_rbw) {
        m_out << "column rbw not found." << std::endl;
        col_not_found = true;
      }
      tools::wroot::base_pntuple::column_string* col_str =
        _base_pntuple->find_column_string("string");
      if (!col_str) {
        m_out << "column string not found." << std::endl;
        col_not_found = true;
      }
      tools::wroot::base_pntuple::column<int>* col_count = _base_pntuple->find_column<int>("count");
      if (!col_count) {
        m_out << "column count not found." << std::endl;
        col_not_found = true;
      }
      tools::wroot::base_pntuple::std_vector_column<double>* col_vec_d =
        _base_pntuple->find_column_vector<double>("vec_d");
      if (!col_vec_d) {
        m_out << "column vec_d not found." << std::endl;
        col_not_found = true;
      }
      tools::wroot::base_pntuple::column_vector_string* col_vec_s =
        _base_pntuple->find_column_vector_string("vec_s");
      if (!col_vec_s) {
        m_out << "column vec_s not found." << std::endl;
        col_not_found = true;
      }
...

      {
        ...
        for (unsigned int count = 0; count < entries; count++) {
          if (m_verbose) {
            if (print_count * tools::uint32(count / print_count) == count)
              m_out << "count " << count << std::endl;
          }

          *col_rgauss = rg(gen);

          if (!col_rbw->fill(rbwf(gen))) {
...

          {
            ...
            vec_s.clear();
            unsigned int number =
              read_check_row_wise ? count % 5 : (unsigned int)(5 * rflat(gen));
            for (unsigned int i = 0; i < number; i++) {
          ...
          if (!_imt_ntuple->add_row(_mutex, main_file))
          {  // it may lock/unlock when passing a basket to a main_ntuple branch.
...
class threads : public tools::threads<thread_args>
{
  ...
  public:
    virtual thread_args* create_args(tools::mutex& a_main_mutex, size_t a_index,
                                     size_t /*a_num_threads*/)
    {
      std::string prefix;
      tools::num2s(a_index, prefix);
      prefix += ":";
      return new thread_args(a_main_mutex, a_index, prefix, m_nbk, m_main_ntuple, m_row_wise,
                             m_row_mode, m_nev, m_megas);
    }

  public:
    threads(std::ostream& a_out, const tools::ntuple_booking& a_nbk,
            tools::wroot::ntuple& a_main_ntuple, bool a_row_wise, bool a_row_mode,
            unsigned int a_nev, unsigned int a_megas, bool a_verbose = false)
  ...
  protected:
    threads(const threads& a_from)
      : parent(a_from), m_nbk(a_from.m_nbk), m_main_ntuple(a_from.m_main_ntuple)
    {}
...
int main(int argc, char** argv)
{
  ...
  bool row_mode = args.is_arg(
    "-row_mode");  // if column_wise (to attempt to have a row like storage in column_wise ntuple).
