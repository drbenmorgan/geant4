...
static bool write_test_root(std::ostream& a_out, bool a_verbose, to_compare_cern_root& a_cmp,
                            const std::string& a_file)
{
...

  {
    ...
    {
      std::mt19937 gen;
      std::normal_distribution<double> rgx(1.0, 2.0);
  ...
  {
    std::mt19937 gen;
    std::normal_distribution<float> rf(1.0, 2.0);
    ...
    h.add_annotation(tools::histo::key_axis_y_title(), "1.4*entries");
    if (a_verbose) {
      a_out << "h1df : " << h.title() << ", all entries " << h.all_entries() << ", entries "
            << h.entries() << ", mean " << h.mean() << ", rms " << h.rms() << std::endl;
    }
    // write :
...
static bool read_test_root(std::ostream& a_out, bool a_verbose, const to_compare_cern_root& a_cmp,
                           const std::string& a_file)
{
...

  {
    ...
    if (!tools::equal(a_out, __FILE__, __LINE__, h->equals_TH(a_cmp.m_rg, tolerance, ::fabs), true))
      return false;
...

  {
    ...
    if (!tools::equal(a_out, __FILE__, __LINE__, h->equals_TH(a_cmp.m_prof, tolerance, ::fabs),
                      true))
      return false;
...

  {
    ...
    if (!tools::equal(a_out, __FILE__, __LINE__, h->equals_TH(a_cmp.m_rgbw, tolerance, ::fabs),
                      true))
      return false;
...

  {
    ...
    if (!tools::equal(a_out, __FILE__, __LINE__, h->equals_TH(a_cmp.m_prof2D, tolerance, ::fabs),
                      true))
      return false;
...

  {
    ...
    if (!tools::equal(a_out, __FILE__, __LINE__, h->equals_TH(a_cmp.m_rggbw, tolerance, ::fabs),
                      true))
      return false;
