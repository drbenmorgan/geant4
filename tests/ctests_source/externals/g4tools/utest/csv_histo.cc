...
static bool write_test_csv(std::ostream& a_out, bool a_verbose, to_compare_csv& a_cmp,
                           const std::string& a_prefix)
{
...

  {
    ...
    if (a_verbose) {
      a_out << "h1d : " << h.title() << ", all entries " << h.all_entries() << ", entries "
            << h.entries() << ", mean " << h.mean() << ", rms " << h.rms() << std::endl;
    }
...

  {
    ...
    tools::histo::p1d h("prof", 100, -5, 5, -2, 2);
    {
      std::mt19937 gen;
      std::normal_distribution<double> rg(1.0, 2.0);
      std::cauchy_distribution<double> rbw(0.0, 0.5);
      for (unsigned int count = 0; count < entries; count++)
        h.fill(rg(gen), rbw(gen), 1);
    }
    if (a_verbose) {
      a_out << "p1d : " << h.title() << ", all entries " << h.all_entries() << ", entries "
            << h.entries() << ", mean " << h.mean() << ", rms " << h.rms() << ", Svw "
            << h.get_Svw() << ", Sv2w " << h.get_Sv2w() << std::endl;
    }
...

  {
    ...
    if (a_verbose) {
      a_out << "p1d_edges : " << h.title() << ", all entries " << h.all_entries() << ", entries "
            << h.entries() << ", mean " << h.mean() << ", rms " << h.rms() << ", Svw "
            << h.get_Svw() << ", Sv2w " << h.get_Sv2w() << std::endl;
    }
...

  {
    ...
    if (a_verbose) {
      a_out << "h2d : " << h.title() << ", all entries " << h.all_entries() << ", entries "
            << h.entries() << ", mean_x " << h.mean_x() << ", rms_x " << h.rms_x() << ", mean_y "
            << h.mean_y() << ", rms_y " << h.rms_y() << std::endl;
    }
...

  {
    ...
    tools::histo::p2d h("prof2D", 100, -5, 5, 100, -5, 5, -2, 2);
    {
      ...
      std::cauchy_distribution<double> rbw(0.0, 0.5);
      for (unsigned int count = 0; count < entries; count++)
        h.fill(rg(gen), rg(gen), rbw(gen), 1);
    }
    if (a_verbose) {
      a_out << "p2d : " << h.title() << ", all entries " << h.all_entries() << ", entries "
            << h.entries() << ", mean_x " << h.mean_x() << ", rms_x " << h.rms_x() << ", mean_y "
            << h.mean_y() << ", rms_y " << h.rms_y() << ", Svw " << h.get_Svw() << ", Sv2w "
            << h.get_Sv2w() << std::endl;
    }
...

  {
    ...
    tools::histo::h3d h("rggbw", 20, -5, 5, 20, -5, 5, 20, -2, 2);
    {
      ...
      std::cauchy_distribution<double> rbw(0.0, 0.5);
      for (unsigned int count = 0; count < entries; count++)
        h.fill(rg(gen), rg(gen), rbw(gen), 0.8);
    }
    // plotting hints :
    h.add_annotation(tools::histo::key_axis_x_title(), "rand gauss");
    ...
    if (a_verbose) {
      a_out << "h3d : " << h.title() << ", all entries " << h.all_entries() << ", entries "
            << h.entries() << ", mean_x " << h.mean_x() << ", rms_x " << h.rms_x() << ", mean_y "
            << h.mean_y() << ", rms_y " << h.rms_y() << ", mean_z " << h.mean_z() << ", rms_z "
            << h.rms_z() << std::endl;
    }
...
static bool read_test_csv(std::ostream& a_out, bool a_verbose, const to_compare_csv& a_cmp,
                          const std::string& a_prefix)
{
  ...
  ////////////////////////////////////////////////////////
  {
    ...
    if (a_verbose) {
      a_out << "h1d : " << h->title() << ", all_entries " << h->all_entries() << ", entries "
            << h->entries() << ", mean " << h->mean() << ", rms " << h->rms() << std::endl;
    }
    if (!tools::equal(a_out, __FILE__, __LINE__, h->title(), a_cmp.rg_title)) return false;
    if (!tools::equal<unsigned int>(a_out, __FILE__, __LINE__, h->all_entries(),
                                    a_cmp.rg_all_entries))
      return false;
    if (!tools::equal<unsigned int>(a_out, __FILE__, __LINE__, h->entries(), a_cmp.rg_entries))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->mean(), a_cmp.rg_mean, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->rms(), a_cmp.rg_rms, tolerance,
                              tools::dfabs))
      return false;
    delete h;
    reader.close();
...

  {
    ...
    if (a_verbose) {
      a_out << "p1d : " << h->title() << ", all_entries " << h->all_entries() << ", entries "
            << h->entries() << ", mean " << h->mean() << ", rms " << h->rms() << std::endl;
    }
    if (!tools::equal(a_out, __FILE__, __LINE__, h->title(), a_cmp.prof_title)) return false;
    if (!tools::equal<unsigned int>(a_out, __FILE__, __LINE__, h->all_entries(),
                                    a_cmp.prof_all_entries))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->mean(), a_cmp.prof_mean, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->rms(), a_cmp.prof_rms, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->get_Svw(), a_cmp.prof_Svw, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->get_Sv2w(), a_cmp.prof_Sv2w, tolerance,
                              tools::dfabs))
      return false;
    delete h;
    reader.close();
...

  {
    ...
    tools::histo::p1d* h = static_cast<tools::histo::p1d*>(obj);
    if (a_verbose) {
      a_out << "p1d_edges : " << h->title() << ", all_entries " << h->all_entries() << ", entries "
            << h->entries() << ", mean " << h->mean() << ", rms " << h->rms() << std::endl;
    }
    if (!tools::equal(a_out, __FILE__, __LINE__, h->title(), a_cmp.prof_edges_title)) return false;
    if (!tools::equal<unsigned int>(a_out, __FILE__, __LINE__, h->all_entries(),
                                    a_cmp.prof_edges_all_entries))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->mean(), a_cmp.prof_edges_mean,
                              tolerance, tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->rms(), a_cmp.prof_edges_rms, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->get_Svw(), a_cmp.prof_edges_Svw,
                              tolerance, tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->get_Sv2w(), a_cmp.prof_edges_Sv2w,
                              tolerance, tools::dfabs))
      return false;
    delete h;
    reader.close();
...

  {
    ...
    if (a_verbose) {
      a_out << "h2d : " << h->title() << ", all_entries " << h->all_entries() << ", entries "
            << h->entries() << ", mean_x " << h->mean_x() << ", rms_x " << h->rms_x() << ", mean_y "
            << h->mean_y() << ", rms_y " << h->rms_y() << std::endl;
    }
    if (!tools::equal(a_out, __FILE__, __LINE__, h->title(), a_cmp.rgbw_title)) return false;
    if (!tools::equal<unsigned int>(a_out, __FILE__, __LINE__, h->all_entries(),
                                    a_cmp.rgbw_all_entries))
      return false;
    // if(!tools::equal<unsigned int>(a_out,__FILE__,__LINE__,h->entries(),a_cmp.rgbw_entries))
    // return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->mean_x(), a_cmp.rgbw_mean_x, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->rms_x(), a_cmp.rgbw_rms_x, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->mean_y(), a_cmp.rgbw_mean_y, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->rms_y(), a_cmp.rgbw_rms_y, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->Sxyw(), a_cmp.rgbw_Sxyw, tolerance,
                              tools::dfabs))
      return false;
    delete h;
    reader.close();
...

  {
    ...
    if (a_verbose) {
      a_out << "p2d : " << h->title() << ", all_entries " << h->all_entries() << ", entries "
            << h->entries() << ", mean_x " << h->mean_x() << ", rms_x " << h->rms_x() << ", mean_y "
            << h->mean_y() << ", rms_y " << h->rms_y() << std::endl;
    }
    if (!tools::equal(a_out, __FILE__, __LINE__, h->title(), a_cmp.prof2D_title)) return false;
    if (!tools::equal<unsigned int>(a_out, __FILE__, __LINE__, h->all_entries(),
                                    a_cmp.prof2D_all_entries))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->mean_x(), a_cmp.prof2D_mean_x,
                              tolerance, tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->rms_x(), a_cmp.prof2D_rms_x, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->mean_y(), a_cmp.prof2D_mean_y,
                              tolerance, tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->rms_y(), a_cmp.prof2D_rms_y, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->get_Svw(), a_cmp.prof2D_Svw, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->get_Sv2w(), a_cmp.prof2D_Sv2w,
                              tolerance, tools::dfabs))
      return false;
    delete h;
    reader.close();
...

  {
    ...
    if (a_verbose) {
      a_out << "h3d : " << h->title() << ", all_entries " << h->all_entries() << ", entries "
            << h->entries() << ", mean_x " << h->mean_x() << ", rms_x " << h->rms_x() << ", mean_y "
            << h->mean_y() << ", rms_y " << h->rms_y() << ", mean_z " << h->mean_z() << ", rms_z "
            << h->rms_z() << std::endl;
    }
    ...
    if (!tools::equal<unsigned int>(a_out, __FILE__, __LINE__, h->all_entries(),
                                    a_cmp.rggbw_all_entries))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->mean_x(), a_cmp.rggbw_mean_x, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->rms_x(), a_cmp.rggbw_rms_x, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->mean_y(), a_cmp.rggbw_mean_y, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->rms_y(), a_cmp.rggbw_rms_y, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->mean_z(), a_cmp.rggbw_mean_z, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->rms_z(), a_cmp.rggbw_rms_z, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->Sxyw(), a_cmp.rggbw_Sxyw, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->Syzw(), a_cmp.rggbw_Syzw, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->Szxw(), a_cmp.rggbw_Szxw, tolerance,
                              tools::dfabs))
      return false;
    delete h;
    reader.close();
