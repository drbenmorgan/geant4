...
bool test_root_ntuple(std::ostream& a_out, bool a_verbose)
{
...

  {
    ...
    nbk.add_column<std::string>("string", user_string);
    wncols++;
    ...
    std::mt19937 gen;
    std::normal_distribution<double> vrg(5, 2);
    std::normal_distribution<double> rg(1, 2);
    std::cauchy_distribution<float> rbwf(0, 0.5);
    std::string stmp;
    for (unsigned int count = 0; count < wentries; count++) {
      double vd = rg(gen);
      hgw.fill(vd);
      user_rgauss = vd;
      TOOLS_TEST_FUNC(col_rbw->fill(rbwf(gen)))
