...
bool test_hdf5_histo(std::ostream& a_out, bool a_verbose)
{
  ...
  {
    std::normal_distribution<double> rg(0.0, 1.0);
  ...
  {
    for (unsigned int count = 0; count < 30000; count++)
      h2.fill(rg(gen), rbw(gen), 0.8);
  }

  tools::histo::p1d p1("Profile", 100, -5, 5, -2, 2);
  {
    for (unsigned int count = 0; count < 10000; count++)
      p1.fill(rg(gen), rbw(gen), 1);
  }

  tools::histo::p2d p2("Profile2D", 100, -5, 5, 100, -5, 5, -2, 2);
  {
    for (unsigned int count = 0; count < 40000; count++)
      p2.fill(rg(gen), rg(gen), rbw(gen), 1);
  }
...

  {
    ...
    if (!tools::equal(a_out, __FILE__, __LINE__,
                      toolx::hdf5::write_atb(histos, "type", "directory"), true))
    {
      ::H5Fclose(_file);
      return false;
    }

    if (!tools::equal(a_out, __FILE__, __LINE__, toolx::hdf5::write_histo(a_out, histos, "h1", h1),
                      true))
    {
      ::H5Gclose(histos);
      ::H5Fclose(_file);
    ...
    if (!tools::equal(a_out, __FILE__, __LINE__, toolx::hdf5::write_histo(a_out, histos, "h2", h2),
                      true))
    {
      ::H5Gclose(histos);
      ::H5Fclose(_file);
    ...
    if (!tools::equal(a_out, __FILE__, __LINE__,
                      toolx::hdf5::write_profile(a_out, histos, "p1", p1), true))
    {
      ::H5Gclose(histos);
      ::H5Fclose(_file);
      return false;
    ...
    if (!tools::equal(a_out, __FILE__, __LINE__,
                      toolx::hdf5::write_profile(a_out, histos, "p2", p2), true))
    {
      ::H5Gclose(histos);
      ::H5Fclose(_file);
      return false;
...

  {
...

    {
      tools::histo::h1d* rh1;
      if (!tools::equal(a_out, __FILE__, __LINE__,
                        toolx::hdf5::read_histo(a_out, histos, "h1", rh1), true))
      {
        ::H5Gclose(histos);
        ::H5Fclose(_file);
        return EXIT_FAILURE;
...

    {
      tools::histo::h2d* rh2;
      if (!tools::equal(a_out, __FILE__, __LINE__,
                        toolx::hdf5::read_histo(a_out, histos, "h2", rh2), true))
      {
        ::H5Gclose(histos);
        ::H5Fclose(_file);
        return false;
...

    {
      tools::histo::p1d* rp1;
      if (!tools::equal(a_out, __FILE__, __LINE__,
                        toolx::hdf5::read_profile(a_out, histos, "p1", rp1), true))
      {
        ::H5Gclose(histos);
        ::H5Fclose(_file);
        return EXIT_FAILURE;
...

    {
      tools::histo::p2d* rp2;
      if (!tools::equal(a_out, __FILE__, __LINE__,
                        toolx::hdf5::read_profile(a_out, histos, "p2", rp2), true))
      {
        ::H5Gclose(histos);
        ::H5Fclose(_file);
        return EXIT_FAILURE;
