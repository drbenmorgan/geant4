...
static bool write_axml(std::ostream& a_out, bool a_verbose, to_compare_axml& a_cmp,
                       const std::string& a_file)
{
  ...
  //////////////////////////////////////////////////////////
  {
    ...
    for (unsigned int count = 0; count < ntu_entries; count++) {
      col_rgauss->fill(rg(gen));
      col_rbw->fill(rbw(gen));
...
static bool read_axml(std::ostream& a_out, bool a_verbose, const to_compare_axml& a_cmp,
                      const std::string& a_file)
{
...

  {
    ...
    for (it = objs.begin(); it != objs.end(); ++it) {
      ...
      if (raxml_out.cls() == tools::histo::h1d::s_class()) {
        ...
        if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->all_entries(), a_cmp.rg_all_entries,
        ...
        if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->all_entries(),
                                  a_cmp.prof_all_entries, tolerance, tools::dfabs))
          return false;
        ...
        if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->rms(), a_cmp.prof_rms, tolerance,
                                  tools::dfabs))
          return false;
        if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->get_Svw(), a_cmp.prof_Svw,
                                  tolerance, tools::dfabs))
          return false;
        if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->get_Sv2w(), a_cmp.prof_Sv2w,
                                  tolerance, tools::dfabs))
          return false;
      }
        ...
        if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->all_entries(),
                                  a_cmp.rgbw_all_entries, tolerance, tools::dfabs))
          return false;
        if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->mean_x(), a_cmp.rgbw_mean_x,
                                  tolerance, tools::dfabs))
          return false;
        if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->rms_x(), a_cmp.rgbw_rms_x,
                                  tolerance, tools::dfabs))
          return false;
        if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->mean_y(), a_cmp.rgbw_mean_y,
                                  tolerance, tools::dfabs))
          return false;
        if (!tools::equal<double>(a_out, __FILE__, __LINE__, h->rms_y(), a_cmp.rgbw_rms_y,
                                  tolerance, tools::dfabs))
          return false;
      }
        ...
        if ((*it).name() == "rg_rbw") {
...

          {
            ...
            for (itc = cols.begin(); itc != cols.end(); ++itc) {
              tools::aida::aida_base_col* acol =
                tools::safe_cast<tools::aida::base_col, tools::aida::aida_base_col>(*(*itc));
              // retain the empty condition to avoid warning about acol being unused.
              if (acol) {
              }
            }
          }
          ...
          }
          {
            ...
            if (col) {
              ...
              if (!tools::equal(a_out, __FILE__, __LINE__, vs, a_cmp.ntu_rg_rbw_strings))
                return false;
            }
          }
        }
          ...
          // h.hprint(a_out);
          if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.all_entries(),
                                    a_cmp.ntu_rg_rbw_2_all_entries, tolerance, tools::dfabs))
            return false;
          if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean(), a_cmp.ntu_rg_rbw_2_mean,
                                    tolerance, tools::dfabs))
            return false;
          if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.rms(), a_cmp.ntu_rg_rbw_2_rms,
                                    tolerance, tools::dfabs))
            return false;
        }
      }
