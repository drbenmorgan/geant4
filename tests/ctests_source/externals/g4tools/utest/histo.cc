...
{
  {
    ...
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.bin_Sw(20), 1.1 * 1000, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean(), 0, tolerance, tools::dfabs))
      return false;
    h.reset();
...

  {
    ...
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.bin_Sw(22, 14), 1.2 * 2000, tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean_x(), 1, tolerance, tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean_y(), 2, tolerance, tools::dfabs))
      return false;
    h.reset();
    if (h.all_entries() != 0) return false;
  }
...
bool test_h1d_rand(std::ostream& a_out, bool a_verbose)
{
  ...
  std::mt19937 gen;
  std::normal_distribution<double> rg(1.0, 0.5);
  H1D h("Gauss", 100, -5, 5);
  {
    for (unsigned int count = 0; count < entries; count++)
      h.fill(rg(gen), 1.4);
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean(), 1, 1e-3, tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.rms(), 0.5, 1e-3, tools::dfabs))
      return false;
  }
  h.reset();
  {
    for (unsigned int count = 0; count < entries; count++)
      h.fill(rg(gen) - 1.0, 2.4);
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean(), 0, 1e-3, tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.rms(), 0.5, 1e-3, tools::dfabs))
      return false;
  }
...
static bool test_p1d_rand(std::ostream& a_out, bool a_verbose)
{
  ...
  unsigned int entries = 2000000;
  std::mt19937 gen;
  std::normal_distribution<double> rgx(1.0, 0.5);
  std::normal_distribution<double> rgy(2.0, 0.4);
  tools::histo::p1d h("Profile", 100, -5, 5);
  {
    for (unsigned int count = 0; count < entries; count++)
      h.fill(rgx(gen), rgy(gen), 1);
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean(), 1, 1e-3, tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.rms(), 0.5, 1e-3, tools::dfabs))
      return false;
...
bool test_h2d_rand(std::ostream& a_out, bool a_verbose)
{
  ...
  std::mt19937 gen;
  std::normal_distribution<double> rgx(1.0, 0.5);
  std::normal_distribution<double> rgy(0.5, 0.2);
  H2D h("Gauss_Gauss", 100, -5, 5, 200, -2, 2);
  for (unsigned int count = 0; count < entries; count++)
    h.fill(rgx(gen), rgy(gen), 0.8);
  if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean_x(), 1, 1e-3, tools::dfabs))
    return false;
  if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.rms_x(), 0.5, 1e-3, tools::dfabs))
    return false;
  if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean_y(), 0.5, 1e-3, tools::dfabs))
    return false;
  if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.rms_y(), 0.2, 1e-3, tools::dfabs))
    return false;
  return true;
}
...
static bool test_h2d_proj(std::ostream& a_out, bool a_verbose)
{
...

  {
    ...
    if (!pj) return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, pj->mean(), 1, 1e-3, tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, pj->rms(), 0.5, 1e-3, tools::dfabs))
      return false;
    delete pj;
  }

  {
    ...
    if (!pj) return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, pj->mean(), 0.5, 1e-3, tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, pj->rms(), 0.2, 1e-3, tools::dfabs))
      return false;
    delete pj;
  }

  {
    ...
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, slice->mean(), 1, 1e-2, tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, slice->rms(), 0.5, 1e-2, tools::dfabs))
      return false;
    delete slice;
  }
...
static bool test_h3d_rand(std::ostream& a_out, bool a_verbose)
{
  ...
  std::normal_distribution<double> rgx(1.0, 0.5);
  std::normal_distribution<double> rgy(0.5, 0.2);
  std::normal_distribution<double> rgz(1.5, 0.3);
  tools::histo::h3d h("Gauss_Gauss_flat", 100, -10, 10, 100, -2, 2, 100, -3, 3);
  for (unsigned int count = 0; count < entries; count++)
    h.fill(rgx(gen), rgy(gen), rgz(gen));
  if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean_x(), 1, 1e-3, tools::dfabs))
    return false;
  if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.rms_x(), 0.5, 1e-3, tools::dfabs))
    return false;
  ...
  if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.rms_y(), 0.2, 1e-3, tools::dfabs))
    return false;
  if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean_z(), 1.5, 1e-3, tools::dfabs))
    return false;
  if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.rms_z(), 0.3, 1e-3, tools::dfabs))
    return false;
  ...
  /////////////////////////////////////////////////////////////////////
  {
...

    {
      ...
      if (!_pj) return false;
      if (!tools::equal<double>(a_out, __FILE__, __LINE__, _pj->mean(), 1.0, 1e-3, tools::dfabs))
        return false;
      if (!tools::equal<double>(a_out, __FILE__, __LINE__, _pj->rms(), 0.5, 1e-3, tools::dfabs))
        return false;
      delete _pj;
    }

    {
      ...
      if (!_pj) return false;
      if (!tools::equal<double>(a_out, __FILE__, __LINE__, _pj->mean(), 0.5, 1e-3, tools::dfabs))
        return false;
      if (!tools::equal<double>(a_out, __FILE__, __LINE__, _pj->rms(), 0.2, 1e-3, tools::dfabs))
        return false;
      delete _pj;
    }
  ...
  /////////////////////////////////////////////////////////////////////
  {
...

    {
      ...
      if (!_pj) return false;
      if (!tools::equal<double>(a_out, __FILE__, __LINE__, _pj->mean(), 1.5, 1e-3, tools::dfabs))
        return false;
      if (!tools::equal<double>(a_out, __FILE__, __LINE__, _pj->rms(), 0.3, 1e-3, tools::dfabs))
        return false;
      delete _pj;
    }
