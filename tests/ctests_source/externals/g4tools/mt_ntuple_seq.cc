...
// This program produces a mt_ntuple_seq.root file by using
// tools::wroot::mt_ntuple_[row,column]_wise but by NOT using multiple threads. The filling logic is
// similar to the mt_ntuple.cpp program but by doing "threads in sequence". It permits to help
// debugging a truely multiple threads situation if needed. It permits to use the tools::mem
// counting.
...
int main(int argc, char** argv)
{
...

  {
    ...
    {  // user variables to create "ref columns" in the main ntuple. (They are not used to fill
       // data) :
      double user_rgauss;
      float user_rbw;
      ...
      main_ntuple = new tools::wroot::ntuple(rfile.dir(), nbk, row_wise);  // owned by the
                                                                           // directory.
      ...
      for (unsigned int ithread = 0; ithread < num_threads; ithread++) {
        ...
        if (!nbk_thread.has_similar_layout(std::cout, nbk)) {
          std::cout << "local thread :"
                    << " not similar booking as the main one." << std::endl;
          break;
        }
        ...
        if (row_wise) {
          ...
          pntuple = new tools::wroot::mt_ntuple_row_wise(
            std::cout, rfile.byte_swap(), rfile.compression(), rfile.dir().seek_directory(),
            *main_branch, basket_size, nbk_thread, verbose);
        }
        else {
          ...
          pntuple = new tools::wroot::mt_ntuple_column_wise(
            std::cout, rfile.byte_swap(), rfile.compression(), rfile.dir().seek_directory(),
            main_branches, basket_sizes, nbk_thread, row_mode, nev, verbose);
        }

        {
          ...
          if (verbose)
            std::cout << "thread : " << ithread << ", user_vec_d_count " << user_vec_d_count
                      << std::endl;
        }
