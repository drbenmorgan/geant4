...
int main(int argc, char** argv)
{
  ...
  if (comm_size < 3) {
    std::cout << "This program requires at least three processes. Only " << comm_size << " given."
              << std::endl;
    return EXIT_FAILURE;
  }
  int rank;
  ...
  // the ntuple_booking should be the same for all ranks (in this program it is not passed through
  // MPI to ranks) :
  double user_rgauss;
  float user_rbw;
  int user_count;
  ...
  if (rank == 0) {
    ...
    bool row_mode = args.is_arg("-row_mode");  // if column_wise (to attempt to have a row like
                                               // storage in column_wise ntuple).
    ...
    //////////////////////////////////////////////////////////
    {
      ...
      if (verbose)
        std::cout << "rank " << rank << " : processor name " << processor_name << std::endl;
      ...
      tools::wroot::ntuple* main_ntuple =
        new tools::wroot::ntuple(rfile.dir(), nbk, row_wise);  // owned by the directory.
      ...
      if (row_wise) {
        ...
        tools_vforcit(tools::wroot::branch*, main_branches, it)
          basket_sizes.push_back((*it)->basket_size());
      }
...

      {
        ...
        if (!_impi.bpack(verbose)) {
          std::cout << "bpack(verbose) failed." << std::endl;
          return EXIT_FAILURE;
        }
        if (!_impi.pack(main_ntuple_id)) {
          std::cout << "pack(ntuple_id) failed." << std::endl;
          return EXIT_FAILURE;
        }
        if (!_impi.bpack(row_wise)) {
          std::cout << "bpack(row_wise) failed." << std::endl;
          return EXIT_FAILURE;
        }
        if (!_impi.bpack(rfile.byte_swap())) {
          std::cout << "bpack(byte_swap) failed." << std::endl;
          return EXIT_FAILURE;
        }
        if (!_impi.pack(rfile.compression())) {
          std::cout << "pack(compression) failed." << std::endl;
          return EXIT_FAILURE;
        }
        if (!_impi.pack(rfile.dir().seek_directory())) {
          std::cout << "pack(seek) failed." << std::endl;
          return EXIT_FAILURE;
        }
        if (row_wise) {
          if (!_impi.pack(basket_size)) {
            std::cout << "pack(basket_size) failed." << std::endl;
            return EXIT_FAILURE;
          }
        }
        else {
          if (!_impi.bpack(row_mode)) {
            std::cout << "bpack(row_mode) failed." << std::endl;
            return EXIT_FAILURE;
          }
          if (!_impi.vpack(basket_sizes)) {
            std::cout << "vpack(basket_sizes) failed." << std::endl;
            return EXIT_FAILURE;
          }
          if (!_impi.pack(nev)) {
            std::cout << "pack(nev) failed." << std::endl;
            return EXIT_FAILURE;
          }
        }
        if (!_impi.pack(num_megas)) {
          std::cout << "pack(num_megas) failed." << std::endl;
          return EXIT_FAILURE;
        }
      ...
      while (true) {
        ...
        if (!_impi.unpack(protocol)) {
          std::cout << "sunpack(protocol) failed." << std::endl;
          return EXIT_FAILURE;
        }

        if (protocol == tools::wroot::mpi_protocol_basket()) {
          tools::uint32 ntuple_id;
          if (!_impi.unpack(ntuple_id)) {
            std::cout << "unpack(ntuple_id) failed." << std::endl;
            return EXIT_FAILURE;
          }
...
#include "read_root.icc"
  ...
  if ((rank == 1) || (rank == 2)) {
...

    {
      ...
      if (!_impi.bunpack(verbose)) {
        std::cout << "bunpack(verbose) failed." << std::endl;
        return EXIT_FAILURE;
      }
      if (!_impi.unpack(main_ntuple_id)) {
        std::cout << "unpack(ntuple_id) failed." << std::endl;
        return EXIT_FAILURE;
      }
      if (!_impi.bunpack(row_wise)) {
        std::cout << "bunpack(row_wise) failed." << std::endl;
        return EXIT_FAILURE;
      }
      if (!_impi.bunpack(byte_swap)) {
        std::cout << "bunpack(byte_swap) failed." << std::endl;
        return EXIT_FAILURE;
      }
      if (!_impi.unpack(compression)) {
        std::cout << "unpack(compression) failed." << std::endl;
        return EXIT_FAILURE;
      }
      if (!_impi.unpack(seek_directory)) {
        std::cout << "unpack(seek) failed." << std::endl;
        return EXIT_FAILURE;
      }
      if (row_wise) {
        if (!_impi.unpack(basket_size)) {
          std::cout << "unpack(basket_size) failed." << std::endl;
          return EXIT_FAILURE;
        }
      }
      else {
        if (!_impi.bunpack(row_mode)) {
          std::cout << "bunpack(row_mode) failed." << std::endl;
          return EXIT_FAILURE;
        }
        if (!_impi.vunpack(basket_sizes)) {
          std::cout << "vunpack(basket_sizes) failed." << std::endl;
          return EXIT_FAILURE;
        }
        if (!_impi.unpack(nev)) {
          std::cout << "unpack(nev) failed." << std::endl;
          return EXIT_FAILURE;
        }
      }
      if (!_impi.unpack(num_megas)) {
        std::cout << "unpack(num_megas) failed." << std::endl;
        return EXIT_FAILURE;
      }
    }
    ...
    if (row_wise) {
      _ntuple =
        new tools::wroot::mpi_ntuple_row_wise(main_ntuple_id, std::cout, byte_swap, compression,
                                              seek_directory, basket_size, nbk, false /*verbose*/);
    }
    else {
      _ntuple = new tools::wroot::mpi_ntuple_column_wise(main_ntuple_id, std::cout, byte_swap,
                                                         compression, seek_directory, basket_sizes,
                                                         nbk, row_mode, nev, false /*verbose*/);
    }
...

    {
      ...
      std::cauchy_distribution<double> rbwd(-1.0, 0.5);
      std::uniform_real_distribution<double> rflat(0.0, 1.0);
      std::string stmp;
      ...
      for (unsigned int count = 0; count < entries; count++) {
...

        {
          user_vec_d.clear();
          unsigned int number =
            read_check_row_wise ? count % 100 : (unsigned int)(10 * rflat(gen));
          for (unsigned int i = 0; i < number; i++)
            user_vec_d.push_back(rg(gen));
        }

        {
          user_vec_s.clear();
          unsigned int number = read_check_row_wise ? count % 5 : (unsigned int)(5 * rflat(gen));
          for (unsigned int i = 0; i < number; i++) {
    ...
    if (!_ntuple->end_fill(_impi, rank_src, tag)) {
    }  // important.
